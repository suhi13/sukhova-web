...
public class UserApiIntegrationTest extends ServicesTestSupport {
  ...
  private static final String CREATED_PASSWORD = "Password1";
  private static final String NEW_CREATED_PASSWORD = "Password2";
...
//  private TestRestTemplate testRestTemplate;
  ...
  public void createUserChangePassword() throws Exception {
      ...
      final HttpEntity<PersistableUser> persistableUser = new HttpEntity<>(newUser, getHeader());

      ReadableUser user;
      final ResponseEntity<ReadableUser> response = testRestTemplate.exchange("/api/v1/private/user/", HttpMethod.POST,
      ...
      userPassword.setPassword(CREATED_PASSWORD);
      userPassword.setChangePassword(NEW_CREATED_PASSWORD);

      final HttpEntity<UserPassword> changePasswordEntity = new HttpEntity<>(userPassword, getHeader());

      final ResponseEntity<Void> changePassword = testRestTemplate.exchange(String.format("/api/v1/private/user/" +
              user.getId() + "/password"), HttpMethod.PATCH, changePasswordEntity, Void.class);
